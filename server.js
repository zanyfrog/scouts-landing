...
const authBaseUrl = String(
	process.env.AUTH_BASE_URL || "http://127.0.0.1:3000",
).replace(/\/+$/, "");
		...
		const proxyReq = http.request(
			targetUrl,
			{
				...
				headers: {
					...
					host: targetUrl.host,
					connection: "close",
					...(baseUrl === authBaseUrl && isLocalRequest(req)
						? { "X-Local-Demo-Login": "true" }
						: {}),
				},
			},
			(proxyRes) => {
...
server.listen(port, () => {
	console.log(
		`Scouts landing server running at http://0.0.0.0:${port}`,
	);
});
