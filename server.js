...
function ensureDataFiles() {
  ...
  if (!fs.existsSync(files.adults)) {
    writeCsv(files.adults, ["id", "name", "relationship", "email"], defaultAdults);
  }

  if (!fs.existsSync(files.adultLeaders)) {
    writeCsv(files.adultLeaders, ["adultId", "role"], defaultAdultLeaders);
  } else {
  ...
  if (!fs.existsSync(files.adultScoutRelationships)) {
    ...
    writeCsv(files.adultScoutRelationships, ["adultId", "scoutId", "relationship", "priority"], buildAdultScoutRelationshipsFromRows(adultsRows, scoutsRows));
  }
...
async function handleApi(req, res) {
  if (req.method === "GET" && req.url === "/api/data") {
    json(res, 200, getDataPayload());
    return true;
  }

  if (req.method === "POST" && req.url === "/api/scouts") {
    const body = JSON.parse((await readBody(req)) || "{}");
    const scouts = Array.isArray(body.scouts) ? body.scouts : [];
    writeCsv(
      files.scouts,
      scoutHeaders,
      scouts.map((scout) => [
  ...
  if (req.method === "POST" && req.url === "/api/adult-leaders") {
    const body = JSON.parse((await readBody(req)) || "{}");
    const adultLeaders = Array.isArray(body.adultLeaders) ? body.adultLeaders : [];
    writeCsv(
      files.adultLeaders,
      ["adultId", "role"],
      adultLeaders.map((adultLeader) => [
        adultLeader.adultId,
        adultLeader.role,
      ])
    );
  ...
  if (req.method === "POST" && req.url === "/api/adult-scout-relationships") {
    const body = JSON.parse((await readBody(req)) || "{}");
    const adultScoutRelationships = Array.isArray(body.adultScoutRelationships) ? body.adultScoutRelationships : [];
    writeCsv(
      files.adultScoutRelationships,
      ["adultId", "scoutId", "relationship", "priority"],
  ...
  if (req.method === "POST" && req.url === "/api/patrols") {
    const body = JSON.parse((await readBody(req)) || "{}");
    const patrols = Array.isArray(body.patrols) ? body.patrols : [];
    writeJson(
      files.patrols,
      patrols.map((patrol) => ({
        name: patrol.name,
